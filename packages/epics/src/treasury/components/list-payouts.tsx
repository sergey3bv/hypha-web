...
import { Tabs, TabsContent, TabsTrigger, TabsList } from '@hypha-platform/ui/server';
  ...
  return (
    <div className='w-full'>
      <div className='flex justify-between items-center mt-4'>
        <Text className='text-lg'>Payouts | $ {formatCurrencyValue(totalValue)}</Text>
        ...
        <Tabs defaultValue="all" className='mt-3'>
          <TabsList>
            ...
            <TabsContent value="all">
              <div className="my-6 flex items-center flex-col w-full">
                <div className='flex flex-col w-full'>
                  {payouts.map((payout) => (
                    <CardPayout status={payout.status} avatar={payout.avatar} name={payout.name} surname={payout.surname} date={payout.date} value={payout.value} symbol={payout.symbol}/>
                  ))}
                </div>
            ...
            <TabsContent value="completed">
              <div className="my-6 flex items-center flex-col w-full">
                <div className='flex flex-col w-full'>
                  {payouts.filter(payout => payout.status === 'completed').map((payout) => (
                    <CardPayout status={payout.status} avatar={payout.avatar} name={payout.name} surname={payout.surname} date={payout.date} value={payout.value} symbol={payout.symbol}/>
                  ))}
                </div>
                <Button onClick={onLoadMore} className="rounded-lg w-fit mt-4" variant="outline" size="sm">
            ...
            <TabsContent value="rejected">
              <div className="my-6 flex items-center flex-col w-full">
                <div className='flex flex-col w-full'>
                  {payouts.filter(payout => payout.status === 'rejected').map((payout) => (
                    <CardPayout status={payout.status} avatar={payout.avatar} name={payout.name} surname={payout.surname} date={payout.date} value={payout.value} symbol={payout.symbol}/>
                  ))}
                </div>
                <Button onClick={onLoadMore} className="rounded-lg w-fit mt-4" variant="outline" size="sm">
