...
interface IDirectory {
    function getVotingPowerSourceContract(uint256 _votingPowerSourceId) external view returns (address);
}
...
interface IExecutor {
    function executeTransaction(address target, uint256 value, bytes memory data) external returns (bool);
}
...
contract DAOProposalsStorage is Initializable {
    ...
    // New storage for nested proposals
