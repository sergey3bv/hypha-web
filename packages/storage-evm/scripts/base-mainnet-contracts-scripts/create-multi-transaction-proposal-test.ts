...
async function testMultiTransactionProposal(): Promise<void> {
  console.log('Starting multi-transaction proposal test...');
  ...
  try {
    ...
    // USDC has 6 decimals
    const fundingAmount = ethers.parseUnits('0.03', 6); // 0.03 USDC - more to support multiple transfers
    ...
    try {
      ...
      if (walletBalance < fundingAmount) {
        console.log(
          `Insufficient USDC balance. Need at least 0.03 USDC to fund the executor.`,
        );
        console.log(
      ...
      console.log(`Sending 0.03 USDC to executor ${executorAddress}...`);
    ...
    // Step 3: Create a Proposal with multiple transactions
    console.log('\nCreating a proposal with THREE USDC transfers...');

    // Helper function to create a USDC transfer transaction
    function createUsdcTransferTransaction(amountInUSDC: string): Transaction {
    ...
    // Create three separate USDC transfer transactions
    const transaction1 = createUsdcTransferTransaction('0.001');
    const transaction2 = createUsdcTransferTransaction('0.0025');
    const transaction3 = createUsdcTransferTransaction('0.0015');
    ...
    const proposalParams: ProposalParams = {
      ...
      transactions: [transaction1, transaction2, transaction3], // Include all three transactions
    };

    console.log('Proposal parameters:', {
      spaceId: proposalParams.spaceId,
      duration: proposalParams.duration,
      transactionCount: proposalParams.transactions.length,
      transactions: proposalParams.transactions.map((t, index) => ({
        index,
        target: t.target,
        value: t.value,
    ...
    console.log('Submitting multi-transaction proposal...');
    try {
      ...
      console.log(`Multi-transaction proposal created with ID: ${proposalId}`);
      ...
      if (proposalData.executed) {
        console.log('Success! The multi-transaction proposal was executed.');

        // Check the wallet's USDC balance after execution
        const walletBalanceAfter = await usdcToken.balanceOf(wallet.address);
        console.log(
          `Wallet USDC balance after execution: ${ethers.formatUnits(
            walletBalanceAfter,
            6,
          )} USDC`,
        );
      ...
      // Error handling remains the same as in the original script
      ...
      try {
        ...
        const tx = await backupDaoProposals.createProposal(proposalParams, {
          gasLimit: Math.floor(Number(backupEstimatedGas) * 1.2),
        });
...
// Run the test
testMultiTransactionProposal().catch(console.error);
